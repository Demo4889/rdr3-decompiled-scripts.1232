...
void func_12()
{
	...
	if (!func_2(256))
	{
		...
		while (iVar1 <= (iVar0 - 1))
		{
			...
			{
				_NAMESPACE71::UI_FEED_CLEAR_CHANNEL(iVar1, 1, 1);
			}
			iVar1++;
		}
	}
...
int func_47()
{
	...
	if (ANIMSCENE::IS_ANIM_SCENE_LOADED(iLocal_60, 1, 0))
	{
...
void func_49(int iParam0)
{
	...
	while (iVar0 <= 31)
	{
		...
		else if (!PED::IS_PED_READY_TO_RENDER(Global_1049161[iVar0]))
		{
		}
		else
		{
...
char* func_61(int iParam0, char* sParam1)
{
	...
	switch (iParam0)
	{
		case -1759663922:
			...
			{
				return HUD::GET_TEXT_SUBSTRING_2(sParam1, HUD::GET_LENGTH_OF_LITERAL_STRING(sParam1));
			}
			break;
		default:
...
void func_85()
{
	...
	switch (iLocal_15)
	{
		...
		case 1:
			...
			else if (ANIMSCENE::IS_ANIM_SCENE_RUNNING(iLocal_60, 0) && ANIMSCENE::GET_ANIM_SCENE_PHASE(iLocal_60) < 0.825f)
			{
			}
			else
			{
